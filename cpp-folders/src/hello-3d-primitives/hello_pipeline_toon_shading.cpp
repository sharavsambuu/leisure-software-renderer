/*
    3D Software Renderer - Threaded Toon Shading (Cel Shading) Pipeline
...
shs::Color toon_fragment_shader(const shs::Varyings& in, const Uniforms& u)
{
    ...
    float intensity;
    if (NdotL > 0.95f)      intensity = 1.0f; 
    else if (NdotL > 0.5f)  intensity = 0.7f; 
    else if (NdotL > 0.25f) intensity = 0.4f; 
    else                    intensity = 0.2f; 

    // 2. Specular (Hard edge)
