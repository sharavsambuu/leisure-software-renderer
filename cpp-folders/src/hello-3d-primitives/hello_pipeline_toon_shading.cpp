/*
    3D Software Renderer - Toon Shading (Cel Shading) Pipeline
    Discrete Lighting Bands + Rim Light
    + FPS Camera Controls (Drag to Look)
    + Static Monkey (No Rotation)
*/
...
// VERTEX SHADER (Standard for Per-Pixel Shading)
shs::Varyings toon_vertex_shader(const glm::vec3& aPos, const glm::vec3& aNormal, const Uniforms& u)
{
    shs::Varyings out;
    out.position = u.mvp * glm::vec4(aPos, 1.0f);
    out.world_pos = glm::vec3(u.model * glm::vec4(aPos, 1.0f));
    // Normal-ийг World Space руу хувиргана
    out.normal = glm::normalize(glm::mat3(glm::transpose(glm::inverse(u.model))) * aNormal);
    out.uv = glm::vec2(0.0f); 
    return out;
}

// FRAGMENT SHADER (Toon / Cel Shading)
shs::Color toon_fragment_shader(const shs::Varyings& in, const Uniforms& u)
{
    ...
    // 1. DIFFUSE INTENSITY CALCULATION
    // Гэрэл гадаргуу дээр хэр хурц тусаж байгааг олно (0.0 - 1.0)
    float NdotL = glm::max(glm::dot(norm, lightDir), 0.0f);
    
    // 2. TOON RAMP (Quantization)
    // Гэрлийн эрчмийг шаталсан утгуудад хуваана
    float intensity;
    if (NdotL > 0.95f)      intensity = 1.0f; // Маш хурц гэрэл
    else if (NdotL > 0.5f)  intensity = 0.7f; // Энгийн гэрэлтэй хэсэг
    else if (NdotL > 0.25f) intensity = 0.4f; // Бүдэг сүүдэр
    else                    intensity = 0.2f; // Гүн сүүдэр

    // 3. SPECULAR (Toon Style)
    // Гялбааг мөн адил зөөлөн уусгахгүй, хурц цэг болгоно
    glm::vec3 halfwayDir = glm::normalize(lightDir + viewDir);
    float NdotH = glm::max(glm::dot(norm, halfwayDir), 0.0f);
    
    // Гялбааны хэмжээг shininess-аар тохируулна
    // Хэрэв тодорхой босгоос давбал шууд 1.0 (цагаан), үгүй бол 0.0 (байхгүй)
    float specular = (pow(NdotH, 32.0f) > 0.5f) ? 0.8f : 0.0f;

    // 4. RIM LIGHT (Optional but good for Toon look)
    // Ирмэгийн гэрэлтүүлэг
    float rimDot = 1.0f - glm::max(glm::dot(viewDir, norm), 0.0f);
    // Ирмэг хэсэгт бөгөөд гэрэлтэй талд байвал тодруулна
    float rimIntensity = (rimDot > 0.7f && NdotL > 0.3f) ? 0.5f : 0.0f;

    // 5. COMBINE
    glm::vec3 objectColorVec = glm::vec3(u.color.r, u.color.g, u.color.b) / 255.0f;
    
    // (Объектын өнгө * Гэрлийн шатлал) + Гялбаа + Ирмэг
    glm::vec3 result = (objectColorVec * intensity) + glm::vec3(specular) + glm::vec3(rimIntensity);
...
public:
    MonkeyObject(glm::vec3 position, glm::vec3 scale, shs::Color color)
    {
        ...
        // Эхлэлдээ бага зэрэг хажуу тийш харсан байна
    ...
    {
        // Хүсэлтийн дагуу эргүүлэхгүй
        // this->rotation_angle += 30.0f * delta_time; 
        (void)delta_time; // Unused variable warning-аас сэргийлэх
    }
...
public:
    HelloScene(shs::Canvas *canvas, Viewer *viewer) 
    {
        ...
        // Toon shading дээр тод өнгө гоё харагддаг (Тод улбар шар)
        //this->scene_objects.push_back(new MonkeyObject(glm::vec3(0.0f, 0.0f, 10.0f), glm::vec3(4.0f), shs::Color{255, 140, 0, 255}));
                    ...
                    shs::Canvas::draw_triangle_pipeline(
                        ...
                        *this->z_buffer,
                        ...
                        [&uniforms](const glm::vec3& p, const glm::vec3& n) {
                            return toon_vertex_shader(p, n, uniforms);
                        },
                        // Fragment Shader: Toon Shading
                        [&uniforms](const shs::Varyings& v) {
                            return toon_fragment_shader(v, uniforms);
                        }
                    );
                }
            }
...
int main(int argc, char* argv[])
{
    ...
    while (!exit)
    {
        ...
        shs::Canvas::fill_pixel(*main_canvas, 0, 0, CANVAS_WIDTH, CANVAS_HEIGHT, shs::Color{30, 30, 40, 255}); // Dark BG
