...
/*
    ...
    PURPOSE: PURE TRANSITION for the renderpath pod.
             ...
             never pod state. Gateway invariant: an invalid compile keeps the
...
#include "shs/domains/renderpath/renderpath.command.hpp"
...
namespace shs::renderpath
{
    ...
    namespace detail
    {
        ...
        // untouched and PATH_SWAP_REJECTED is emitted (gateway invariant).
    ...
    // compiler + caps are pure value context; events land on the caller's
    // frame arena via the pmr event vector.
    inline void renderpath_gateway(
        RenderPathPodState& state,
