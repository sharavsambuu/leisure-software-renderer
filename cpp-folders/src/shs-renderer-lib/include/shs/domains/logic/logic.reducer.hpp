...
/*
    ...
    FILE: logic.reducer.hpp
    MODULE: domains/logic
    PURPOSE: CORE 4. REDUCER — the table-driven FSM transition (R5b, P4.1
...
namespace shs
{
    ...
    template <typename TStateId>
    inline void reduce_fsm(
        FsmState<TStateId>&                 state,
        std::span<const FsmAction<TStateId>> actions,
        const FsmDesc<TStateId>&            desc,
        const FsmInputs<TStateId>&          inputs,
        std::pmr::vector<FsmEvent<TStateId>>& events)
    {
        (void)inputs;
        for (const auto& action : actions)
        {
            if (const auto* start = std::get_if<FsmStart<TStateId>>(&action))
            {
            ...
            else if (const auto* signal = std::get_if<FsmSignal<TStateId>>(&action))
            {
            ...
            else if (const auto* force = std::get_if<FsmForce<TStateId>>(&action))
            {
            ...
            else if (const auto* tick = std::get_if<FsmTick>(&action))
            {
                if (!state.started) continue;
