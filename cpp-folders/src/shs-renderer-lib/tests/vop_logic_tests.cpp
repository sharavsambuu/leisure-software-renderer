...
#include "shs/domains/logic/logic.reducer.hpp"
...
namespace
{
    ...
    using Action = shs::FsmAction<Id>;
    using Event  = shs::FsmEvent<Id>;
    using Desc   = shs::FsmDesc<Id>;
    using Inputs = shs::FsmInputs<Id>;
    ...
    std::pmr::vector<Event> run(State& s, const Desc& d, const std::vector<Action>& actions,
                                std::pmr::monotonic_buffer_resource& arena)
    {
        std::pmr::vector<Event> events{&arena};
        shs::reduce_fsm(s, std::span<const Action>{actions.data(), actions.size()}, d, Inputs{}, events);
        return events;
    }
    ...
    bool test_replay_deterministic()
    {
        ...
        auto reduce = [&](State& s, std::span<const Action> a, const Inputs& in,
                          std::pmr::vector<Event>& e)
        {
            shs::reduce_fsm(s, a, desc, in, e);
        };
        return shs::pod_test::replay_is_deterministic<State, Action, Inputs, Event>(
            reduce, s0, std::span<const Action>{actions.data(), actions.size()}, Inputs{});
    }
    ...
    bool test_empty_log_stable()
    {
        ...
        return shs::pod_test::empty_log_is_stable<State, Action, Inputs, Event>(
            reduce, s0, Inputs{});
    }
} // namespace
