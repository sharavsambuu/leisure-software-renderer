...
namespace
{
    ...
    bool test_follow_known_answers()
    {
        ...
        shs::camera::follow_target(rig, glm::vec3(10.0f, 0.0f, 0.0f), glm::vec3(0.0f), 1.0f, 0.016f);
        if (rig.pos != glm::vec3(10.0f, 0.0f, 0.0f)) return false;

        shs::camera::CameraRig rig2{};
        rig2.pos = glm::vec3(1.0f);
        shs::camera::follow_target(rig2, glm::vec3(10.0f, 0.0f, 0.0f), glm::vec3(0.0f), 0.0f, 0.016f);
    ...
    bool test_light_camera_fit()
    {
        ...
        const shs::camera::LightCamera a = shs::camera::build_dir_light_camera_aabb(sun, box);
        const shs::camera::LightCamera b = shs::camera::build_dir_light_camera_aabb(sun, box);
