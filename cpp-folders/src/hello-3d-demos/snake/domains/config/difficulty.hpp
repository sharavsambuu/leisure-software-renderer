...
namespace snake::config {
    ...
    struct Difficulty {
        bool solid_walls;   // true = walls are impenetrable (die on touch); false = soft walls (bounce)
        uint32_t speed_ticks;  // ticks per second of game loop (higher = faster). Default ~15 → ~6.67 ticks/sec.
    };
