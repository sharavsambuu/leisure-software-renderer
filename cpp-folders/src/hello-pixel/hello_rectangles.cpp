...
void canvas_save_png(const std::string& filename, std::vector<std::vector<Pixel>>& canvas, int width, int height) {
    if (SDL_Init(SDL_INIT_VIDEO) < 0) { std::cerr << "SDL_Init Error: " << SDL_GetError() << std::endl; return; }
    int imgFlags = IMG_INIT_PNG;
    if ((IMG_Init(imgFlags) & imgFlags) != imgFlags) { std::cerr << "IMG_Init Error: " << IMG_GetError() << std::endl; SDL_Quit(); return; }

     SDL_Surface* surface = SDL_CreateRGBSurface(0, width, height, 32, 0, 0, 0, 0);

    if (!surface) { std::cerr << "SDL_CreateRGBSurface Error: " << SDL_GetError() << std::endl; IMG_Quit(); SDL_Quit(); return; }
    ...
    if (IMG_SavePNG(surface, filename.c_str()) < 0) { std::cerr << "IMG_SavePNG Error: " << IMG_GetError() << std::endl;
    } else { std::cout << "Image saved successfully." << std::endl; }
...
int main() {
    ...
    canvas_generate_random(random_canvas, canvas_width, canvas_height);
    canvas_fill_color(white_canvas , canvas_width, canvas_height, color_white);
    canvas_fill_color(red_canvas   , canvas_width, canvas_height, color_red  );
    canvas_fill_color(canvas_canvas, canvas_width, canvas_height, color_black);

    canvas_set_color(canvas_canvas, 10, 10, color_red);
    canvas_set_color(canvas_canvas, 20, 20, color_red);
    canvas_set_color(canvas_canvas, 30, 30, color_red);
    canvas_set_color(canvas_canvas,  5, 60, color_white);
    canvas_flip_horizontally(canvas_canvas); // origin at the left bottom corner of the canvas

    canvas_save_png("random_canvas.png", random_canvas, canvas_width, canvas_height);
    canvas_save_png("white_canvas.png" , white_canvas , canvas_width, canvas_height);
    canvas_save_png("red_canvas.png"   , red_canvas   , canvas_width, canvas_height);
    canvas_save_png("canvas_canvas.png", canvas_canvas, canvas_width, canvas_height);
