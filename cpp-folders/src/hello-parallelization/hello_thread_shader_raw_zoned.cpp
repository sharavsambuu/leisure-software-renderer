...
int main(int argc, char* argv[])
{
    ...
    while (!exit_loop)
    {
        ...
        // ============================================================
        // Дэлгэцийг 8x8=64 хэсэгт хуваахын оронд
        // Мөрөөр нь (Horizontal Slicing) 8 хэсэгт хуваана.
        // thread үүсгэх зардал 8 дахин багасана.
        // ============================================================
        
        std::vector<std::thread> thread_pool;
        int height_per_thread = CANVAS_HEIGHT / CONCURRENCY_COUNT;

        for (int i = 0; i < CONCURRENCY_COUNT; i++) {
            
            // Аль мөрнөөс аль мөр хүртэл зурахыг тооцоолно
            int start_y = i * height_per_thread;
            int end_y   = (i + 1) * height_per_thread;
            
            // Сүүлийн thread үлдэгдэл пикселүүдийг гүйцээх ёстой
            if (i == CONCURRENCY_COUNT - 1) {
                end_y = CANVAS_HEIGHT;
            }
            ...
            thread_pool.emplace_back([start_y, end_y, time_accumulator, main_canvas]() {
                for (int y = start_y; y < end_y; y++) {
                    for (int x = 0; x < CANVAS_WIDTH; x++) {
                        ...
                        // Mutex хэрэггүй. Thread бүр өөр өөр мөр (Y) дээр бичиж байгаа.
        ...
        // Бүх thread ажлаа дуустал хүлээнэ (Join)
        for (auto &thread : thread_pool)
        {
            if (thread.joinable()) thread.join();
        }

        // Санамсаргүй пиксел зурж шалгах
    ...
    // MEMORY CLEANUP
    // Дараалал чухал: Эхлээд delete хийнэ, дараа нь nullptr онооно.
    delete main_canvas;
    main_canvas = nullptr; 
