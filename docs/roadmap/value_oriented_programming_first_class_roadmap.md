...
- Added value-oriented input action/gateway API in `shs/input/value_commands.hpp`.
- Added core runtime input-latch gateway in `shs/input/value_input_latch.hpp`.
...
- Added command-to-action conversion path in `ICommand` + `CommandProcessor` batch gateway flow.
- Added `shs_renderer_tests` target with deterministic checks for gateways and pipeline planning.
- Migrated `HelloPassPlumbing` from command-object execution to value actions + gateway flow.
...
- Extended `shs_renderer_tests` with pipeline request-gate coverage (invalid request skips pass execution).
...
- Added executable VOP boundary gate script (`tools/check_kdba_boundaries.sh`) and CMake target `shs_renderer_boundary_check` to block planner-side backend includes and `dynamic_cast` policy branching.
...
- Migrated difficult `exp-plumbing` camera/input paths to gateway flow:
  - `HelloPassBasicsVulkan` now uses `PlatformInputState -> InputState -> RuntimeAction -> runtime_state_gateway(...)` for free-camera motion/look/quit.
  - `HelloRenderingPaths` now routes movement/look through the same gateway flow at `update_frame_data(...)`, while preserving existing event/toggle behavior.
- Completed VOP migration for remaining `exp-plumbing` demo input/control paths:
  - `HelloPassBasics` now uses gateway-driven free-camera updates (`InputState -> RuntimeAction -> RuntimeState`) instead of direct per-frame camera mutation.
  ...
  - `hello_jolt_integration` and `HelloSoftwareTriangle` now process quit via value actions/gateway flow for consistency with VOP-first runtime handling.
...
5. Add hard enforcement gates. (In progress)
   - Remaining work:
     - Wire CTest VOP gates (`shs_renderer_boundary_check`, `shs_renderer_tests`) into CI so planner-boundary and deterministic core checks fail automatically.
...
1. Standardize `std::span` for non-owning contiguous inputs in planner/gateway APIs.
...
9. Adopt `std::expected` (or `tl::expected`) to formalize error states in planner diagnostics instead of asserting or crashing.
   - Scope (KDBA amendment 2026-09-16, supersedes tier doctrine): **KDBA Kleisli doctrine** — atomic `A -> expected<B, DomainError>` arrows via `.and_then()` / `.transform()` / `.or_else()` everywhere including the gateway core (`expected<Step{NextState, Events}, ClosedEnum>`; no switch-case monoliths; no phantom flags); see `../spec/value_oriented_programming.md` §8 "KDBA Kleisli Pipeline Doctrine". The lib baselines C++23 (Constitution I §10). The superseded leaf-seams paragraph is preserved verbatim at `../outdated/vop-track-leaf-seams-scope-2026-09-16.md`.
...
1. Refactor forward classic demo into action/gateway/planner/runtime stages.
...
1. Add deterministic golden tests for planners and gateways.
...
## Phase 8: Data-Oriented Data Layout (DOD)
...
4. Deterministic test coverage for gateways and planners.

## Exit Criteria

1. Planning layers are pure and reproducible.
2. Runtime is the only side-effecting layer.
3. Demos use value snapshots and action gateways end to end.
4. Legacy mutation-heavy paths are removed.
5. Planner/gateway layers avoid hidden mutable state (`static` cache, dynamic type branching).
