...
Depends on 2-6.
- [x] Move root `shs::` public symbols into owner namespaces in separate slices.
  Existing `shs::camera`, `shs::input`, `shs::logic` and `shs::renderpath` APIs
  need no redundant namespace layer.
  (Done 2026-09-17: 13 CTest-gated slices with `tools/namespace_cutover.py` —
  core, geometry, task, platform, input, scene, camera, gfx, lighting, sky,
  frame+render, resources, renderpath (+10b: `renderpath/execution`, the tool's
  forwarder-skip was too broad), rhi, app. Each slice: full build + 34-35/35
  CTest + inventory regeneration + commit. `shs::app` pre-existed as a
  non-inline namespace so it uses plain nesting + root using-declarations
  instead of `inline`; rhi `Context` fwd decl re-pointed to `shs::app`. Left
  unwrapped by design: `app/runtime_state.hpp` (no-op edge-zone alias, no
  declarations); `domains/`+`execution/` forwarders excluded by the tool.)
- [x] Keep explicit old-name aliases/wrappers where safe; test overload lookup,
  ...
  (Done 2026-09-17: inline-namespace compat layer + explicit root
  using-declarations for `app`. New CTest `shs_renderer_namespace_cutover_tests`
  pins: `is_same` same-entity across old/new spellings (types, aliases,
  `Context`, `RuntimeState`), overload resolution via old/new spellings
  (`create_render_backend` enum/string overloads + default arguments), ADL
  through a foreign namespace on `shs::render::RenderBackendType`, stable
  serialization identifiers (`RenderBackendType` underlying ids 0/1/2 +
  name/parse round-trip), and mixed legacy-forwarder/canonical includes in one
  TU. 35/35 CTest green.)
- [x] Migrate repository consumers/tutorials/catalogs/tools; publish a mapping
  and release-specific deprecation schedule.
  (Done 2026-09-17: `tools/root_spelling_migration.py` resolved 320 of 392
  root-spelling tokens used by repo consumers to a single owning module
  (10 hand-verified overrides, e.g. `shs::Context` -> `shs::app::Context`) and
  rewrote 92 consumer files (tests + all `exps-*` tutorials); the remaining 72
  tokens resolve to symbols defined outside this library and stay. Mapping +
  release-specific deprecation schedule (0.1.x compat -> 0.2.0 deprecate ->
  next breaking release remove forwarders) published in
  `docs/backlog/namespace_cutover_mapping.md`, generated by
  `tools/namespace_spelling_mapping.py` (517 unambiguous symbols). Full build
...
- [x] Reject new legacy includes in canonical sources; retire linter exemptions
  ...
  includes in canonical headers (`legacy include in library header: ...`);
  `tests/header_migration_tests.py` positive/negative-tests the gate via
  CTest `shs_renderer_header_migration_tests`; canonical sources contain no
  `shs/domains/` or `shs/execution/` includes (verified). The
  `engine_header_migration_manifest.json` + this doc's Done notes + the new
  mapping doc archive the migration history rather than deleting it.)
