...
- [x] **C1.1 `shs/core/contract_guardrails.hpp`** — DONE 2026-09-17 (60 lines exactly): handler seam (`contract_kind`, `contract_violation`, `set_contract_violation_handler`) + macros; enforced → check + handler, release → `[[assume]]` via `__has_cpp_attribute(assume)`, `__cpp_contracts` branch reserved for C4.3. Self-containment gate green on GCC 13.3 / C++23 both ways. `SHS_PRE`/`SHS_POST`/`SHS_CONTRACT_ASSERT`; debug (`SHS_CONTRACTS_ENFORCED`) → check + handler; release → `[[assume]]`; C++26 branch keyed on `__cpp_contracts` (may compile to nothing until a compiler supports it). DoD: include-clean (IWYU), compiles on GCC 13.3 / C++23 with and without `-DSHS_CONTRACTS_ENFORCED`, passes the self-containment gate. **RULING (owner, 2026-09-17): the in-house bridge is the ratified contract mechanism — no third-party contract library is adopted.** Evaluated against C++23 options: GSL `Expects`/`Ensures` has no runtime-pluggable handler seam (termination-policy macros only, no kind/expr/file/line capture — would break the negative-test twins and the replay-parity CTest); Boost.Contract is heavy, dormant, and wrong-shaped for a 65-line need; `std::expected`/railway is the value tier, not the edge-contract tier (P1 split). C++23 offers no native contracts (that is C++26); the ladder already is the maximal C++23 form (feature-test `[[assume]]` fold, handler seam, parity test). Sanctioned replacement path: the C++26 native switch (C4.3, facts in education doc §7.1) — the bridge is designed to shrink to a shim there, not to be swapped for a dependency.
...
- [x] **C2.2 `RenderPathCompiler` transition asserts** — DONE 2026-09-17, P1-legal placement: the technique-mode transition table moved to its pure leaf home (`render_path_recipe.hpp`); the compiler asserts a compiled plan's technique is the table image of its mode (`SHS_CONTRACT_ASSERT` in `render_path_compiler.hpp`, allowlisted pure leaf). Negative test: hand-mismatched technique/mode pair fires kind=assertion. `map_rejection` (reason leg) and `apply_runtime_toggle` (toggle leg) stay INFO-tracked closed-enum dispatches — **RULING (owner, 2026-09-17, ratified in the W-E bookkeeping pass): legs stay INFO-tracked as-is.** They already are named, single-purpose arrows (`map_rejection`, `apply_runtime_toggle` in `renderpath.gateway.hpp`) with exhaustive 1:1 closed-enum switches and total fallbacks; no annotation and no move. Rationale: moving the compiler→pod mappings into event/contract headers would put translation work in the value tier against the P1 edge/value split, and the legs are structural (closed enums + total functions), matching the W-D finding that closed vocabularies need compile-time pins, not runtime rails. The earlier "ruled at the C2.4 retro" wording was imprecise — the retro deferred this without ruling; the gap is now closed with the explicit ruling above.
...
- [x] **C2.4 Pilot retro** — DONE 2026-09-17 (W-C joint retro, owner rulings granted). **Findings:** (1) the annotations caught a real tiering violation — stating the C2.2 invariant forced the transition table out of `render_path_presets.hpp` into its pure-leaf home; (2) the C2.1 negative test caught a masked hand-break (shadow-map rule rejected the plan upstream before the commit rim); (3) exactly one pure, allocation-free helper was needed (`renderpath_plan_pass_chain_registered`) — single-expression conditions scale fine when helpers are P1-legal pure leaves. **Rulings (owner, 2026-09-17):** (a) sweep beyond renderpath GRANTED — W-D opens, logic pod first; (b) P2 expected-rail exclusivity ENACTED as a blocking gate (evidence: all 4 gateways already conformant — 0 throws in seams, rims return Step values); (c) P5 closed-variant exhaustiveness ENACTED (gate + trailing `static_assert` tails in the renderpath/logic dispatches — the audit found real gaps: if-constexpr chains with no tail silently ignore new alternatives). Landed as `tools/check_gateway_rails.sh` + `shs_renderer_gateway_rails_negative_test` + boundary gate 9. DoD met: retro notes recorded; sweep unblocked.
...
- [x] **C4.1 Feature-test discipline** — verified 2026-09-17: `grep -rn "cpp_contracts" cpp-folders/src/shs-renderer-lib` shows only the bridge (`contract_guardrails.hpp` L9 doc comment + L48 `#if`); zero compiler-name branches (`__GNUC__`/`__clang__`/`_MSC_VER`/`__INTEL_COMPILER` — 0 hits in include/src/tests); `__has_cpp_attribute(assume)` (feature-test, not sniffing) exists only in the bridge. Standing discipline: keep every expansion keyed on feature tests.
...
- [ ] **C4.3 Switch run plan (blocked on C4.2 ruling)** — flip the `__cpp_contracts` branch, rewrite sites (mechanical: single-expression conditions per bridge rule 2), move handler to `<contracts>` plumbing, replay-parity CTest green, retire emulation. **Runbook drafted 2026-09-17** (`cpp26_native_switch_runbook.md`): complete 14-site inventory enumerated (8 production, 6 test; all single-expression), steps, acceptance gates, rollback pre-verified — execution is one mechanical commit once the baseline ruling lands. DoD: run plan executed and marked DONE with commit hash; bridge shrinks to a shim.
