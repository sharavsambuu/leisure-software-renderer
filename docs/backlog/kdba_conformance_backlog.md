...
- [ ] **K1.1a Port-order scope amendment (2026-09-19, KP-0)** — the six remaining
      ports (`geometry → lighting → sky → scene → resources → gfx`) are re-scoped
      by the vocabulary audit in [`kleisli_port_scope_todo.md`](kleisli_port_scope_todo.md):
      all six command/event vocabularies are `std::variant<std::monostate>` with
      zero consumers (lawful §6.1 monostate scaffolding), so there is nothing to
      port until real intents exist. `gfx`/`resources`/`scene` rims are
      **demand-driven** — the named trigger is the R5b registry/store edge
      migration; `geometry` is a pure value-lib leaf (no rim, module-class
      amendment §6.1); `sky` defers until a backend sky-realization edge appears;
      `lighting` stays retired (step 4.5, `pod_identifier_law.md`). Honors this
      entry's own banner: no signature-only ports, no invented vacuous errors.
