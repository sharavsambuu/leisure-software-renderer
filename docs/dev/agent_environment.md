...
| Filesystem command with **no** explicit path (`ls`, `cat`, `git status`, `cmake ..`) | ⚠️ Silently runs against `C:\Windows` — wrong results, no error | Never rely on it. Use file tools or an explicit WSL path. |
