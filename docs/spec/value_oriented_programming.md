...
13. **Module Boundary Include-Direction (amendment, 2026-09-17)**: The public-header dependency graph is a DAG flowing value-ward. Value and contract headers never include adapter, edge, executor, platform, driver, or third-party SDK headers. Adapters include their owning contracts plus their external dependencies — never the reverse — and neutral contract headers compile without Vulkan, SDL, Assimp or Jolt headers. Canonical public headers are directly include-clean (IWYU): depending on a transitive include for a used name (`std::span`, GLM types) is a bug, not a convenience. Mechanically enforced by `check_include_graph.py` and `check_kdba_boundaries.sh` (§10).
...
- [x] Module boundary law (2026-09-17 amendment, Rules 13–16): include-graph DAG gate with the shrink-only exceptions manifest (`check_include_graph.py` + `engine_include_exceptions.json`; any unlisted violation FAILs), canonical-header migration checker (`check_header_migration.py`), and the content-hashed header inventory (`inventory_headers.py`). Required validation state: full build + CTest with the optional SDK options (SDL2, Assimp) OFF, headless (lavapipe + `VK_LAYER_KHRONOS_validation` where Vulkan is exercised).
